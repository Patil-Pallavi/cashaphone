...
?>
        ...
        <form action="<?php echo JRoute::_($this->formLink) ; ?>" method="post" name="visform" id="<?php echo $this->visforms->parentFormId; ?>" class="visform <?php echo $this->visforms->formlayout; ?> <?php echo $this->visforms->formCSSclass; echo (($this->visforms->formlayout == "bthorizontal") || ($this->visforms->formlayout == "bt3horizontal")) ? " form-horizontal " : "";?>"<?php if($this->upload == true) { ?> enctype="multipart/form-data"<?php } ?> role="form">
<?php
//add a progressbar
...
for ($f = 1; $f < $this->steps + 1; $f++)
{
    ...
    if ($f === $this->steps)
    {
        ...
        if (empty($this->visforms->displaysummarypage))
        {
            ...
            $layout = new JLayoutFile('visforms.footers.btdefault.nosummary', null);
            $layout->setOptions(array('component' => 'com_visforms'));
        ...
        {
            $layout = new JLayoutFile('visforms.footers.btdefault.withsummary', null);
            $layout->setOptions(array('component' => 'com_visforms'));
