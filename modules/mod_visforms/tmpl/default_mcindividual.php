...
?>
    ...
    <form action="<?php echo Juri::base(true) . '/' . htmlspecialchars($formLink, ENT_COMPAT, 'UTF-8'); ?>" method="post" name="visform" id="<?php echo $visforms->parentFormId; ?>" class="mcindividual visform <?php echo $visforms->formCSSclass; ?>"<?php if($upload == true) { ?> enctype="multipart/form-data"<?php } ?> role="form">
<?php
//add a progressbar
...
for ($f = 1; $f < $steps + 1; $f++)
{
    ...
    if ($f === $steps)
    {
        ...
        {
            $layout = new JLayoutFile('visforms.footers.mcindividual.nosummary', null);
            $layout->setOptions(array('component' => 'com_visforms'));
        ...
        {
            $layout = new JLayoutFile('visforms.footers.mcindividual.withsummary', null);
            $layout->setOptions(array('component' => 'com_visforms'));
